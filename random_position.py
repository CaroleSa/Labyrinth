...
class random_position:
    def __init__(self):
        line_liste = [line_1, line_2, line_3, line_4, line_5, line_6, line_7, line_8,
             line_9, line_10, line_11, line_12, line_13
]
        random_line = choice(line_liste)
        line_finale=[]
        ...
        for i, elt in enumerate(random_line):     # va permettre de parcourir tous les élèments de la ligne définie dans les paramètres 
            if elt == "6": 
                index_random_line = line_liste.index(random_line)                  # si un des élèments correspond au number_picture demandé,
                line_finale.append(((i*40), (index_random_line+1)*40))     # les coordonnées de son emplacement s'ajoute à ma liste vide
        self.random_location = choice(line_finale)
        i+=1  
